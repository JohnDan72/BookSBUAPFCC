...
//METHOD GET PARA TODA LA GESTIÓN DE LA PAGINACIÓN
    ...
    $total_pages = $modelLibro->getTotalPages($no_of_records_per_page,$_SESSION['filtro_actual']);	//total de páginas de acuerdo a la info de la DB
    if ($numPage>$total_pages) {$numPage = 1; $offset = ($numPage-1) * $no_of_records_per_page;} //seguridad si ocurre un error por url 	
    $dataCurrentPage = $modelLibro->getDataCurrentPage($offset,$no_of_records_per_page,$_SESSION['filtro_actual']);	//se obtiene la información de la página actual
						    ...
						    <div id="modal<?= $libro['Id']?>" class="modal modal-fixed-footer">
							    ...
							    <div class="modal-footer">
							    	<div class="center">
						    			...
						    			<a href="#!" class="modal-close waves-effect waves-light btn-flat blue darken-1 grey-text text-lighten-5">Realizar el Pedido</a>
							    	</div> 
							    </div>
						    </div>
						<?php
